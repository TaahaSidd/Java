...
2. Reference Variables - Used to refer objects.
    ...
    eg - Student s = new Student(); -> s is the reference variable.
...
1. If the value of the variable varies from object to object such type of variables are called Instance variables.
...
4. Instance variables are created at the time of object creation, and destroyed at the time of object destruction hence the scope of IV is same as the scope of object.

5. IV will be stored in the heap memo as a part of object.

6. IV cant be axseed directly from static method but we can access by using object reference, but we can access IV directly from instance area.
...
2. In the case of IV for every object a separate copy will be created but in the case of static variables a single copy will be created at class level and shared by every object of the class.

3. Static variables should be declared directly in the class but outside of any method, block or constructor

4. SV will be created at the time of class loading and destroyed at time of class unloading, hence scope of SV is exactly same as scope of class.

5. SV are stored in method area.

6. We can access SV either by object reference or class name but its recommended tp access it by class name, withtin the same class it is not req to use class name and we can access directly.
...
1. Variables declared inside a method is called Local Variables.

2. Other names are Temporary,Stack & Automatic variable.

3. LV are stored in Stack memory,

4. Lv will be created while executing the block in which its declared, once the block execution is completed the LV will be destroyed when the block is destroyed its declared in.

5. For LV JVM doesn't provide default values. We should perform init explicitly before using it. If we are not using LV then init is not req.
...
IV and SV can be accessed by multiple threads simultaneously and hence these are not threat safe, but in the case of local variable for every thread a separate copy will be created and hence LV thread safe
